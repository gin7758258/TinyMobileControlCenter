package me.antinomy.hibernate.dao;
...
public abstract class AbstractDao<T extends BaseEntity, PK extends java.io.Serializable> {
    // 日志输出类
    protected static final Logger LOGGER = Util.logger;
    ...
    public PK save(T entity) {
        entity = formatDefaultValue(entity);
    ...
    private T formatDefaultValue(T entity) {
        if (entity.getLastModifyUser() == null || entity.getLastModifyUser().length() == 0) {
            entity.setLastModifyUser("SYSTEM");
        }
        if (entity.getLastModifyModel() == null || entity.getLastModifyModel().length() == 0) {
            entity.setLastModifyModel("SYSTEM");
        }
        entity.setLastModifyDate(new Timestamp(System.currentTimeMillis()));
        return entity;
    }
